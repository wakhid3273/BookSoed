<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Order Masuk — Seller</h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($orders->isEmpty())
                <div class="bg-white rounded-xl shadow p-12 text-center text-gray-400">
                    ...
                    <p class="text-lg">Belum ada order yang masuk.</p>
                </div>
            @else
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 border-b">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-gray-500 font-medium">Buyer</th>
                                <th class="px-4 py-3 text-left text-gray-500 font-medium">Tanggal</th>
                                <th class="px-4 py-3 text-left text-gray-500 font-medium">Buku</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($orders as $order)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-4 text-gray-700">{{ $order->buyer->name }}</td>
                                    <td class="px-4 py-4 text-gray-500">{{ $order->order_date->format('d M Y') }}</td>
                                    <td class="px-4 py-4 text-gray-600">
                                        @foreach ($order->items as $item)
                                            <p class="text-xs">{{ $item->book->title ?? '-' }}</p>
                                        @endforeach
                                    </td>
